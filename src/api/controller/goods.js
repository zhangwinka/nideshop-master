...
module.exports = class extends Base {
  ...
  async findbyidAction() {
    ...
    const goodsList = await this.model('goods').field(['id', 'name', 'list_pic_url', 'retail_price', 'goods_brief']).where({'category_id': goodsId}).select();
    return this.success({
      goods: goodsList
    });
  }
   ...
   *查询是否本周推荐
   * */
  async findbyishotAction() {
    const goodsList = await this.model('goods').field(['id', 'name', 'list_pic_url', 'retail_price', 'goods_brief']).where({is_hot: 1}).select();
    return this.success({
      newGoods: goodsList
    });
  }
